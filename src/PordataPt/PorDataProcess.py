...
class PorDataProcess:
    def __init__(self):
        base = Path("..") / ".." / "Bases"
        self.folderPathPorData = base / "PorData"
        self.folderPathInePt = base / "InePt"
        self.filePathBaseAgregada = (
            self.folderPathPorData / "base_agregada" / "estatisticas_agregadas.csv"
        )
        self.dfs = []
    ...
    def read_json_file(file_path):
        """Return the parsed JSON from ``file_path``.
        ...
        try:
            ...
            logger.error("Failed to decode JSON in %s", file_path)
            raise exc
        ...
        for root, dirs, files in os.walk(folder_path):
            for file in files:
                if file.endswith(".csv"):
                    ...
                    try:
                        df = pd.read_csv(
                            file_path, sep=",", encoding="utf-8", low_memory=False
                        )
                        dataframes.append(
                            {
    ...
    def trata_df_cultura(df_cultura_cinemas):
        df_cultura_cinemas = df_cultura_cinemas[
            df_cultura_cinemas["03. Âmbito Geográfico"].notnull()
        ]
        last_year = df_cultura_cinemas["01. Ano"].max()
        df_cultura_cinemas = df_cultura_cinemas[
            df_cultura_cinemas["01. Ano"] == last_year
        ]
        ...
        df_cultura_cinemas = df_cultura_cinemas.rename(
            columns={
                "01. Ano": "Ano",
    ...
    def adiciona_df_tratado(self, categoria, nome, df):
        """Store processed ``df`` with its category and name."""
    ...
    def lista_cidades_cinemas(df_cultura_cinemas):
        """Return ``df_cultura_cinemas`` aggregated and sorted by number of cinemas."""

        result = (
            df_cultura_cinemas.groupby(["Região", "Âmbito Geográfico"])
            .agg({"Valor": "sum"})
            .reset_index()
        )
        result = result.sort_values(by="Valor", ascending=False).reset_index(drop=True)
        return result
    ...
    def trata_df_populacao_residente(df_populaccao_residente):
        df_populaccao_residente = df_populaccao_residente[
            df_populaccao_residente["04. Âmbito Geográfico"].notnull()
        ]
        df_populaccao_residente = df_populaccao_residente[
        ...
        df_populaccao_residente = df_populaccao_residente[
            df_populaccao_residente["01. Ano"] == last_year
        ]
        df_populaccao_residente_sexo = df_populaccao_residente[
            df_populaccao_residente["06. Filtro 2"] == "Total"
        ]
        ...
        df_populaccao_residente_faixa = df_populaccao_residente[
            df_populaccao_residente["05. Filtro 1"] == "Total"
        ]
        ...
        df_populacao_final = pd.merge(
            df_sexo_pivot, df_faixa_pivot, on=["03. Nome Região (Portugal)"], how="left"
        )
        df_populacao_final.reset_index(inplace=True)

        df_populacao_final = df_populacao_final.rename(
            columns={
                "01. Ano_x": "Ano",
        ...
        df_populacao_final["Total Crianças"] = (
            df_populacao_final["0 - 4 anos"] + df_populacao_final["5 - 9 anos"]
        )
        df_populacao_final["Total Adolescentes"] = (
            df_populacao_final["10 - 14 anos"] + df_populacao_final["15 - 19 anos"]
        )
        df_populacao_final["Total Jovens Adultos"] = df_populacao_final["20 - 24 anos"]
        df_populacao_final["Total Adultos"] = df_populacao_final[
            [
                "25 - 29 anos",
                "30 - 34 anos",
                "35 - 39 anos",
        ...
        df_populacao_final["Total Idosos"] = df_populacao_final[
            [
                "60 - 64 anos",
                "65 - 69 anos",
                "70 - 74 anos",
        ...
        df_populacao_final["Proporção Crianças"] = (
            df_populacao_final["Total Crianças"] / df_populacao_final["Total População"]
        ) * 100
        df_populacao_final["Proporção Adolescentes"] = (
            df_populacao_final["Total Adolescentes"]
            / df_populacao_final["Total População"]
        ) * 100
        df_populacao_final["Proporção Jovens Adultos"] = (
            df_populacao_final["Total Jovens Adultos"]
            / df_populacao_final["Total População"]
        ) * 100
        df_populacao_final["Proporção Adultos"] = (
            df_populacao_final["Total Adultos"] / df_populacao_final["Total População"]
        ) * 100
        df_populacao_final["Proporção Idosos"] = (
            df_populacao_final["Total Idosos"] / df_populacao_final["Total População"]
        ) * 100
    ...
    def cidades_por_grupo(df, categoria="Todos", n=5):
        ...
        for cat in categorias_list:
            cidade_ordenada = (
                df[["Ano", "Região", categorias[cat]]]
                .sort_values(by=categorias[cat], ascending=False)
                .head(n)
            )
    ...
    def trata_df_populacao_densidade(df_densidade_populacional):
        ...
        df_densidade_populacional = df_densidade_populacional[
            df_densidade_populacional["03. Nome Região (Portugal)"].notnull()
        ]

        last_year = df_densidade_populacional["01. Ano"].max()
        df_densidade_populacional = df_densidade_populacional[
            df_densidade_populacional["01. Ano"] == last_year
        ]

        df_densidade_populacional = df_densidade_populacional[
            df_densidade_populacional["03. Nome Região (Portugal)"] != "Portugal"
        ]

        df_densidade_populacional = df_densidade_populacional[
            [
                "01. Ano",
                "03. Nome Região (Portugal)",
                "04. Âmbito Geográfico",
                "10. Valor",
            ]
        ]

        df_densidade_populacional.rename(
            columns={
                "01. Ano": "Ano",
                "03. Nome Região (Portugal)": "Região",
    ...
    def trata_crimes_catetoria(df_seguranca_crime_catetoria):
        ...
        df_seguranca_crime_catetoria = df_seguranca_crime_catetoria.dropna(
            axis=1, how="all"
        )
        df_seguranca_crime_catetoria = df_seguranca_crime_catetoria[
            df_seguranca_crime_catetoria["02. Nome Região (Portugal)"] != "Portugal"
        ]

        last_year = df_seguranca_crime_catetoria["01. Ano"].max()
        df_seguranca_crime_catetoria = df_seguranca_crime_catetoria[
            df_seguranca_crime_catetoria["01. Ano"] == last_year
        ]
        ...
        df_seguranca_crime_catetoria.rename(
            columns={
                "01. Ano": "Ano",
                "02. Nome Região (Portugal)": "Região",
                "03. Âmbito Geográfico": "Âmbito Geográfico",
        ...
        df_seguranca_crime_catetoria = df_seguranca_crime_catetoria[
            df_seguranca_crime_catetoria["Valor"].notnull()
        ]
    ...
    def trata_crimes_tipo(df_seg_crimes_tipo):
        ...
        df_seg_crimes_tipo = df_seg_crimes_tipo[
            df_seg_crimes_tipo["03. Nome Região (Portugal)"] != "Portugal"
        ]
        df_seg_crimes_tipo = df_seg_crimes_tipo[
            df_seg_crimes_tipo["03. Nome Região (Portugal)"].notna()
        ]

        last_year = df_seg_crimes_tipo["01. Ano"].max()
        df_seg_crimes_tipo = df_seg_crimes_tipo[
            df_seg_crimes_tipo["01. Ano"] == last_year
        ]

        df_seg_crimes_tipo = df_seg_crimes_tipo.drop(
            columns=["09. Símbolo", "08. Escala"], axis=1
        )
    ...
    def trata_hospitais(df_saude_hospitais):
        ...
        df_saude_hospitais = df_saude_hospitais[
            df_saude_hospitais["02. Nome Região (Portugal)"] != "Portugal"
        ]

        last_year = df_saude_hospitais["01. Ano"].max()
        df_saude_hospitais = df_saude_hospitais[
            df_saude_hospitais["01. Ano"] == last_year
        ]

        df_saude_hospitais.drop(columns=["08. Símbolo"], inplace=True)

        df_saude_hospitais.rename(
            columns={
                "01. Ano": "Ano",
                "02. Nome Região (Portugal)": "Região",
                "03. Âmbito Geográfico": "Âmbito Geográfico",
    ...
    def trata_farmacias(df_saude_farmacias):
        ...
        df_saude_farmacias = df_saude_farmacias[
            df_saude_farmacias["02. Nome Região (Portugal)"] != "Portugal"
        ]
        ...
        df_saude_farmacias = df_saude_farmacias[
            df_saude_farmacias["01. Ano"] == last_year
        ]

        df_saude_farmacias.rename(
            columns={
                "01. Ano": "Ano",
                "02. Nome Região (Portugal)": "Região",
                "03. Âmbito Geográfico": "Âmbito Geográfico",
    ...
    def trata_ensino_n_superior(df_edu_ensino_n_superior, last_year=None):
        ...
        df_edu_ensino_n_superior = df_edu_ensino_n_superior[
            df_edu_ensino_n_superior["02. Nome Região (Portugal)"] != "Portugal"
        ]

        df_edu_ensino_n_superior = df_edu_ensino_n_superior.drop(
            columns=["08. Símbolo"]
        )
        if last_year == None:
            last_year = df_edu_ensino_n_superior["01. Ano"].max()
        df_edu_ensino_n_superior = df_edu_ensino_n_superior[
            df_edu_ensino_n_superior["01. Ano"] == last_year
        ]

        df_edu_ensino_n_superior.rename(
            columns={
                "01. Ano": "Ano",
                "02. Nome Região (Portugal)": "Região",
                "03. Âmbito Geográfico": "Âmbito Geográfico",
        ...
        df_edu_nivel = df_edu_ensino_n_superior[
            df_edu_ensino_n_superior["Tipo de Escola"] == "Total"
        ]
        ...
        df_edu_tipo = df_edu_ensino_n_superior[
            df_edu_ensino_n_superior["Nível de Ensino"] == "Total"
        ]
        ...
        df_edu_escola_final = pd.merge(
            df_edu_escola_tipo, df_edu_escola_nivel, on=["Ano", "Região"], how="left"
        )
        df_edu_escola_final.reset_index(inplace=True)

        df_edu_escola_final.drop(columns=["index", "Total_x"], inplace=True)
        df_edu_escola_final.rename(
            columns={"Total_y": "Total Estabelecimentos"}, inplace=True
        )
    ...
    def trata_ensino_superior(df_edu_ensino_superior):
        ...
        df_edu_ensino_superior = df_edu_ensino_superior[
            df_edu_ensino_superior["02. Nome Região (Portugal)"] != "Portugal"
        ]
        ...
        df_edu_ensino_superior = df_edu_ensino_superior[
            df_edu_ensino_superior["01. Ano"] == last_year
        ]

        df_edu_ensino_superior.rename(
            columns={
                "01. Ano": "Ano",
                "02. Nome Região (Portugal)": "Região",
                "03. Âmbito Geográfico": "Âmbito Geográfico",
    ...
    def trata_econ_depositos(df_econ_depositos):
        ...
        df_econ_depositos = df_econ_depositos[
            df_econ_depositos["02. Nome Região (Portugal)"] != "Portugal"
        ]
    ...
    def trata_econ_bancos(df_econ_estabelecimentos_bancos):
        ...
        df_econ_estabelecimentos_bancos = df_econ_estabelecimentos_bancos.dropna(
            axis=1, how="all"
        )

        df_econ_estabelecimentos_bancos = df_econ_estabelecimentos_bancos[
            df_econ_estabelecimentos_bancos["02. Nome Região (Portugal)"] != "Portugal"
        ]

        last_year = df_econ_estabelecimentos_bancos["01. Ano"].max()
        df_econ_estabelecimentos_bancos = df_econ_estabelecimentos_bancos[
            df_econ_estabelecimentos_bancos["01. Ano"] == last_year
        ]

        df_econ_estabelecimentos_bancos.rename(
            columns={
                "01. Ano": "Ano",
                "02. Nome Região (Portugal)": "Região",
                "03. Âmbito Geográfico": "Âmbito Geográfico",
    ...
    def trata_df_desemprego(df_desemprego):
        ...
        df_desemprego = df_desemprego[
            df_desemprego["02. Nome Região (Portugal)"] != "Portugal"
        ]
    ...
    def trata_populacao_empregada_escolaridade(df_polacao_empregada):
        ...
        df_polacao_empregada = df_polacao_empregada[
            df_polacao_empregada["02. Nome Região (Portugal)"] != "Portugal"
        ]
        df_polacao_empregada = df_polacao_empregada.drop(columns=["08. Símbolo"])

        last_year = df_polacao_empregada["01. Ano"].max()
        df_polacao_empregada = df_polacao_empregada[
            df_polacao_empregada["01. Ano"] == last_year
        ]

        df_polacao_empregada.rename(
            columns={
                "01. Ano": "Ano",
                "02. Nome Região (Portugal)": "Região",
                "03. Âmbito Geográfico": "Âmbito Geográfico",
    ...
    def trata_populacao_estrangeira(df_populacao_estrangeira):
        ...
        df_populacao_estrangeira = df_populacao_estrangeira[
            df_populacao_estrangeira["02. Nome Região (Portugal)"] != "Portugal"
        ]

        last_year = df_populacao_estrangeira["01. Ano"].max()
        df_populacao_estrangeira = df_populacao_estrangeira[
            df_populacao_estrangeira["01. Ano"] == last_year
        ]

        df_populacao_estrangeira = df_populacao_estrangeira.drop(
            columns=["08. Símbolo"]
        )

        df_populacao_estrangeira.rename(
            columns={
                "01. Ano": "Ano",
                "02. Nome Região (Portugal)": "Região",
                "03. Âmbito Geográfico": "Âmbito Geográfico",
    ...
    def trata_populacao_empregada_ramo(df_polacao_empregada_ramo):
        ...
        df_polacao_empregada_ramo = df_polacao_empregada_ramo[
            df_polacao_empregada_ramo["02. Nome Região (Portugal)"] != "Portugal"
        ]

        last_year = df_polacao_empregada_ramo["01. Ano"].max()
        df_polacao_empregada_ramo = df_polacao_empregada_ramo[
            df_polacao_empregada_ramo["01. Ano"] == last_year
        ]

        df_polacao_empregada_ramo = df_polacao_empregada_ramo.drop(
            columns=["08. Símbolo"]
        )

        df_polacao_empregada_ramo.rename(
            columns={
                "01. Ano": "Ano",
                "02. Nome Região (Portugal)": "Região",
                "03. Âmbito Geográfico": "Âmbito Geográfico",
    ...
    def trata_Renda_estrangeira(df_Renda_estrangeira):
        df_Renda_estrangeira = df_Renda_estrangeira.dropna(axis=1, how="all")
        df_Renda_estrangeira = df_Renda_estrangeira[
            df_Renda_estrangeira["02. Nome Região (Portugal)"] != "Portugal"
        ]

        last_year = df_Renda_estrangeira["01. Ano"].max()
        df_Renda_estrangeira = df_Renda_estrangeira[
            df_Renda_estrangeira["01. Ano"] == last_year
        ]

        df_Renda_estrangeira = df_Renda_estrangeira.drop(columns=["08. Símbolo"])

        df_Renda_estrangeira.rename(
            columns={
                "01. Ano": "Ano",
                "02. Nome Região (Portugal)": "Região",
                "03. Âmbito Geográfico": "Âmbito Geográfico",
        ...
        df_Renda_estrangeira = df_Renda_estrangeira.astype(
            {
                "Ano": "int",
                "Região": "str",
                "Âmbito Geográfico": "str",
        ...
        df_Renda_estrangeira = (
            df_Renda_estrangeira.pivot_table(
                index=["Ano", "Região"],
                columns="Nacionalidade",
                values="Valor",
                aggfunc="sum",
        ...
        return df_Renda_estrangeira

    @staticmethod
    def trata_moradia_edificios(df_moradia_edificios):
        df_moradia_edificios = df_moradia_edificios.dropna(axis=1, how="all")
        df_moradia_edificios = df_moradia_edificios[
            df_moradia_edificios["02. Nome Região (Portugal)"] != "Portugal"
        ]

        last_year = df_moradia_edificios["01. Ano"].max()
        df_moradia_edificios = df_moradia_edificios[
            df_moradia_edificios["01. Ano"] == last_year
        ]

        df_moradia_edificios.rename(
            columns={
                "01. Ano": "Ano",
                "02. Nome Região (Portugal)": "Região",
                "03. Âmbito Geográfico": "Âmbito Geográfico",
        ...
        df_moradia_tipo = df_moradia_edificios[
            df_moradia_edificios["Fim que se destina"] == "Total"
        ]
        ...
        df_moradia_destino = df_moradia_edificios[
            df_moradia_edificios["Tipo de Obra"] == "Total"
        ]
        ...
        df_moradia_edificios_final = pd.merge(
            df_moradia_edificios_destino,
            df_moradia_edificios_tipo,
            on=["Ano", "Região"],
            how="left",
        )
    ...
    def trata_construcoes_novas(df_construcoes_novas):
        ...
        df_construcoes_novas = df_construcoes_novas[
            df_construcoes_novas["02. Nome Região (Portugal)"] != "Portugal"
        ]

        last_year = df_construcoes_novas["01. Ano"].max()
        df_construcoes_novas = df_construcoes_novas[
            df_construcoes_novas["01. Ano"] == last_year
        ]

        df_construcoes_novas.rename(
            columns={
                "01. Ano": "Ano",
                "02. Nome Região (Portugal)": "Região",
                "03. Âmbito Geográfico": "Âmbito Geográfico",
    ...
    def trata_sal_ganho_medio_mensal(df_sal_ganho_medio_mensal):
        ...
        df_sal_ganho_medio_mensal = df_sal_ganho_medio_mensal[
            df_sal_ganho_medio_mensal["02. Nome Região (Portugal)"] != "Portugal"
        ]

        last_year = df_sal_ganho_medio_mensal["01. Ano"].max()
        df_sal_ganho_medio_mensal = df_sal_ganho_medio_mensal[
            df_sal_ganho_medio_mensal["01. Ano"] == last_year
        ]

        df_sal_ganho_medio_mensal.drop(columns=["08. Símbolo"], inplace=True)

        df_sal_ganho_medio_mensal.rename(
            columns={
                "01. Ano": "Ano",
                "02. Nome Região (Portugal)": "Região",
                "03. Âmbito Geográfico": "Âmbito Geográfico",
    ...
    def trata_sal_ganho_medio_mensal_escol(df_sal_ganho_medio_mensal_escol):
        df_sal_ganho_medio_mensal_escol = df_sal_ganho_medio_mensal_escol.dropna(
            axis=1, how="all"
        )
        df_sal_ganho_medio_mensal_escol = df_sal_ganho_medio_mensal_escol[
            df_sal_ganho_medio_mensal_escol["02. Nome Região (Portugal)"] != "Portugal"
        ]

        last_year = df_sal_ganho_medio_mensal_escol["01. Ano"].max()
        df_sal_ganho_medio_mensal_escol = df_sal_ganho_medio_mensal_escol[
            df_sal_ganho_medio_mensal_escol["01. Ano"] == last_year
        ]

        df_sal_ganho_medio_mensal_escol.drop(columns=["08. Símbolo"], inplace=True)

        df_sal_ganho_medio_mensal_escol.rename(
            columns={
                "01. Ano": "Ano",
                "02. Nome Região (Portugal)": "Região",
                "03. Âmbito Geográfico": "Âmbito Geográfico",
    ...
    def trata_econ_poder_compra():
        ...
        df_econ_poder_compra = df_econ_poder_compra.drop(0, axis=0).reset_index(
            drop=True
        )

        df_econ_poder_compra = df_econ_poder_compra[
            df_econ_poder_compra["Âmbito Geográfico"] == "Município"
        ]

        df_econ_poder_compra = df_econ_poder_compra[
            ["Âmbito Geográfico", "Anos", df_econ_poder_compra.columns[-1]]
        ]

        ultima_coluna = df_econ_poder_compra.columns[-1]
        df_econ_poder_compra.rename(
            columns={
        ...
        df_econ_poder_compra = df_econ_poder_compra.astype(
            {"Âmbito Geográfico": "str", "Região": "str", "Valor": "float"}
        )
    ...
    def salva_base_agregada(self, df_final):
        df_final.columns = [unidecode(col) for col in df_final.columns]
        df_formatado = self.formatar_numericos(df_final)
        df_formatado.to_csv(self.filePathBaseAgregada, index=False)
        del df_formatado
    ...
    def trata_seg_taxa_crime():
        file_path = (
            ...
            / "InePt"
            / "seguranca"
            / "INE_PT-Taxa-de-criminalidade.xls"
        )
    ...
    def trata_mora_valor_renda():
        ...
        df_mora_valor_rendas = df_mora_valor_rendas[
            df_mora_valor_rendas["Codigo"].str.len() == 7
        ]

        # Criar nova coluna com valor estimado para apartamento de 100m²
        df_mora_valor_rendas["Valor apartamento 100m²"] = (
            df_mora_valor_rendas["Valor mediano das rendas por m2"] * 100
        )

        # Criar nova coluna com valor estimado para apartamento de 80m²
        df_mora_valor_rendas["Valor apartamento 80m²"] = (
            df_mora_valor_rendas["Valor mediano das rendas por m2"] * 80
        )
...
if __name__ == "__main__":
    ...
    dataframes = porDataProcess.read_csv_files_from_folders(
        porDataProcess.folderPathPorData
    )

    # cultura ------------------------------------------------------------------
    df_cultura_cinemas = porDataProcess.find_df_by_name(
        dataframes, "623-recintos-de-cinema"
    )
    df_cultura_cinemas = porDataProcess.trata_df_cultura(df_cultura_cinemas)
    porDataProcess.adiciona_df_tratado(
        "Cultura", "recintos-de-cinema", df_cultura_cinemas
    )

    df_cultura_espetaculos = porDataProcess.find_df_by_name(
        dataframes, "631-sessoes-de-espetaculos-ao-vivo"
    )
    df_cultura_espetaculos = porDataProcess.trata_df_cultura(df_cultura_espetaculos)
    porDataProcess.adiciona_df_tratado(
        "Cultura", "sessoes-de-espetaculos-ao-vivo", df_cultura_espetaculos
    )

    df_populaccao_residente = porDataProcess.find_df_by_name(
        dataframes, "1-populacao-residente-por-sexo-e-grupo-etario"
    )
    df_populaccao_residente = porDataProcess.trata_df_populacao_residente(
        df_populaccao_residente
    )
    porDataProcess.adiciona_df_tratado(
        "Populacso",
        "populacao-residente-por-sexo-e-grupo-etario",
        df_populaccao_residente,
    )

    df_densidade_populacional = porDataProcess.find_df_by_name(
        dataframes, "4-densidade-populacional"
    )
    df_densidade_populacional = porDataProcess.trata_df_populacao_densidade(
        df_densidade_populacional
    )
    porDataProcess.adiciona_df_tratado(
        "Populacao", "densidade-populacional", df_densidade_populacional
    )

    df_populacao_estrangeira = porDataProcess.find_df_by_name(
        dataframes, "354-populacao-estrangeira-com-estatuto-legal-de-residente"
    )
    df_populacao_estrangeira = porDataProcess.trata_populacao_estrangeira(
        df_populacao_estrangeira
    )
    porDataProcess.adiciona_df_tratado(
        "Populacao",
        "populacao-estrangeira-com-estatuto-legal-de-residente",
        df_populacao_estrangeira,
    )

    df_seguranca_crime_catetoria = porDataProcess.find_df_by_name(
        dataframes, "560-crimes-por-categoria"
    )
    df_seg_categoria = porDataProcess.trata_crimes_catetoria(
        df_seguranca_crime_catetoria
    )
    porDataProcess.adiciona_df_tratado(
        "Segurança", "crimes-por-categoria", df_seg_categoria
    )

    df_seg_crimes_tipo = porDataProcess.find_df_by_name(
        dataframes, "559-crimes-registados-pelas-policias-por-tipo-de-crime"
    )
    df_seg_crimes_tipo = porDataProcess.trata_crimes_tipo(df_seg_crimes_tipo)
    porDataProcess.adiciona_df_tratado(
        "Segurança",
        "crimes-registados-pelas-policias-por-tipo-de-crime",
        df_seg_crimes_tipo,
    )
    ...
    df_saude_hospitais = porDataProcess.find_df_by_name(
        dataframes, "470-hospitais-por-natureza-institucional"
    )
    df_saude_hospitais = porDataProcess.trata_hospitais(df_saude_hospitais)
    porDataProcess.adiciona_df_tratado(
        "Saude", "hospitais-por-natureza-institucional", df_saude_hospitais
    )
    ...
    df_edu_ensino_n_superior = porDataProcess.find_df_by_name(
        dataframes, "379-estabelecimentos-de-ensino-nao-superior-por-0"
    )
    df_edu_escola_final = porDataProcess.trata_ensino_n_superior(
        df_edu_ensino_n_superior, 2023
    )
    porDataProcess.adiciona_df_tratado(
        "Educacao", "estabelecimentos-de-ensino-nao-superior-por-0", df_edu_escola_final
    )

    df_edu_ensino_superior = porDataProcess.find_df_by_name(
        dataframes, "375-estabelecimentos-de-ensino-superior-por-subsistema"
    )
    df_edu_ensino_superior = porDataProcess.trata_ensino_superior(
        df_edu_ensino_superior
    )
    porDataProcess.adiciona_df_tratado(
        "Educacao",
        "estabelecimentos-de-ensino-superior-por-subsistema",
        df_edu_ensino_superior,
    )

    df_econ_depositos = porDataProcess.find_df_by_name(
        dataframes, "294-depositos-de-clientes-nos-bancos-caixas-economicas-e"
    )
    df_econ_depositos = porDataProcess.trata_econ_depositos(df_econ_depositos)
    porDataProcess.adiciona_df_tratado(
        "Economia",
        "depositos-de-clientes-nos-bancos-caixas-economicas-e",
        df_econ_depositos,
    )

    df_econ_estabelecimentos_bancos = porDataProcess.find_df_by_name(
        dataframes, "295-estabelecimentos-de-bancos-caixas-economicas-e-caixas"
    )
    df_econ_estabelecimentos_bancos = porDataProcess.trata_econ_bancos(
        df_econ_estabelecimentos_bancos
    )
    porDataProcess.adiciona_df_tratado(
        "Economia",
        "estabelecimentos-de-bancos-caixas-economicas-e-caixas",
        df_econ_estabelecimentos_bancos,
    )

    df_desemprego = porDataProcess.find_df_by_name(
        dataframes, "439-desemprego-registado-nos-centros-de-emprego-por-grup"
    )
    df_desemprego = porDataProcess.trata_df_desemprego(df_desemprego)
    porDataProcess.adiciona_df_tratado(
        "Emprego", "desemprego-registado-nos-centros-de-emprego-por-grup", df_desemprego
    )

    df_polacao_empregada = porDataProcess.find_df_by_name(
        dataframes, "445-populacao-empregada-por-conta-de-outrem-por-nivel-de"
    )
    df_polacao_empregada = porDataProcess.trata_populacao_empregada_escolaridade(
        df_polacao_empregada
    )
    porDataProcess.adiciona_df_tratado(
        "Emprego",
        "populacao-empregada-por-conta-de-outrem-por-nivel-de",
        df_polacao_empregada,
    )

    df_polacao_empregada_ramo = porDataProcess.find_df_by_name(
        dataframes, "845-Pessoal-ao-servico-nas-empresas-por-ramo-de-atividade"
    )
    df_polacao_empregada_ramo = porDataProcess.trata_populacao_empregada_ramo(
        df_polacao_empregada_ramo
    )
    porDataProcess.adiciona_df_tratado(
        "Emprego",
        "Pessoal-ao-servico-nas-empresas-por-ramo-de-atividade",
        df_polacao_empregada_ramo,
    )

    df_moradia_edificios = porDataProcess.find_df_by_name(
        dataframes, "978-edificios-concluidos-por-tipo-de-obra-e-fim-que-se"
    )
    df_moradia_edificios = porDataProcess.trata_moradia_edificios(df_moradia_edificios)
    porDataProcess.adiciona_df_tratado(
        "Moradia",
        "edificios-concluidos-por-tipo-de-obra-e-fim-que-se",
        df_moradia_edificios,
    )

    df_construcoes_novas = porDataProcess.find_df_by_name(
        dataframes, "980-fogos-concluidos-em-construcoes-novas-para-habitacao"
    )
    df_construcoes_novas = porDataProcess.trata_construcoes_novas(df_construcoes_novas)
    porDataProcess.adiciona_df_tratado(
        "Moradia",
        "fogos-concluidos-em-construcoes-novas-para-habitacao",
        df_construcoes_novas,
    )

    df_mora_valor_rendas = porDataProcess.trata_mora_valor_renda()
    porDataProcess.adiciona_df_tratado(
        "Moradia", "Valor Medio Arrendamento", df_mora_valor_rendas
    )

    df_sal_ganho_medio_mensal = porDataProcess.find_df_by_name(
        dataframes, "581-ganho-medio-mensal"
    )
    df_sal_ganho_medio_mensal = porDataProcess.trata_sal_ganho_medio_mensal(
        df_sal_ganho_medio_mensal
    )
    porDataProcess.adiciona_df_tratado(
        "Renda", "ganho-medio-mensal", df_sal_ganho_medio_mensal
    )

    df_sal_ganho_medio_mensal_escol = porDataProcess.find_df_by_name(
        dataframes, "582-ganho-medio-mensal-por-nivel-de-escolaridade"
    )
    df_sal_ganho_medio_mensal_escol = porDataProcess.trata_sal_ganho_medio_mensal_escol(
        df_sal_ganho_medio_mensal_escol
    )
    porDataProcess.adiciona_df_tratado(
        "Renda",
        "ganho-medio-mensal-por-nivel-de-escolaridade",
        df_sal_ganho_medio_mensal_escol,
    )
    ...
    df_cultura.drop(
        columns=["Âmbito Geográfico_x", "Âmbito Geográfico_y"], inplace=True
    )
    ...
    # saude
    df_saude_hospitais = porDataProcess.find_df_by_name(
        dfs, "hospitais-por-natureza-institucional"
    )
    df_farmacias = porDataProcess.find_df_by_name(dfs, "farmacias")

    df_farmacias = df_farmacias.rename(columns={"Valor": "Qtd.Farmacias"})
    df_saude_hospitais = df_saude_hospitais.rename(
        columns={"Parceria público-privada": "Qtd.Hospitais.Publico.Privado"}
    )
    df_saude_hospitais = df_saude_hospitais.rename(
        columns={"Privado": "Qtd.Hospitais.Privado"}
    )
    df_saude_hospitais = df_saude_hospitais.rename(
        columns={"Público": "Qtd.Hospitais.Publico"}
    )
    df_saude_hospitais = df_saude_hospitais.rename(
        columns={"Total": "Qtd.Hospitais.Total"}
    )
    ...
    # educacao
    df_ensino_n_superior = porDataProcess.find_df_by_name(
        dfs, "estabelecimentos-de-ensino-nao-superior-por-0"
    )
    df_ensino_superior = porDataProcess.find_df_by_name(
        dfs, "estabelecimentos-de-ensino-superior-por-subsistema"
    )
    df_ensino_superior = df_ensino_superior.rename(
        columns={"Privado": "Qtd.Ensino.Superior.Privado"}
    )
    df_ensino_superior = df_ensino_superior.rename(
        columns={"Público": "Qtd.Ensino.Superior.Publico"}
    )
    df_ensino_superior = df_ensino_superior.rename(
        columns={"Total": "Qtd.Ensino.Superior.Total"}
    )

    df_ensino_n_superior = df_ensino_n_superior.rename(
        columns={"Privado": "Qtd.Ensino.NSuperior.Privado"}
    )
    df_ensino_n_superior = df_ensino_n_superior.rename(
        columns={"Público": "Qtd.Ensino.NSuperior.Publico"}
    )
    df_ensino_n_superior = df_ensino_n_superior.rename(
        columns={"Escola artística": "Qtd.Ensino.NSuperior.artistica"}
    )
    df_ensino_n_superior = df_ensino_n_superior.rename(
        columns={"Escola básica": "Qtd.Ensino.NSuperior.basica"}
    )
    df_ensino_n_superior = df_ensino_n_superior.rename(
        columns={"Escola básica e secundária": "Qtd.Ensino.NSuperior.basica.secundaria"}
    )
    df_ensino_n_superior = df_ensino_n_superior.rename(
        columns={"Escola profissional": "Qtd.Ensino.NSuperior.profissional"}
    )
    df_ensino_n_superior = df_ensino_n_superior.rename(
        columns={"Escola secundária": "Qtd.Ensino.NSuperior.secundaria"}
    )
    df_ensino_n_superior = df_ensino_n_superior.rename(
        columns={"Jardim-de-infância": "Qtd.Ensino.NSuperior.Jardim_de_infância"}
    )
    df_ensino_n_superior = df_ensino_n_superior.rename(
        columns={"Total Estabelecimentos": "Qtd.Ensino.NSuperior.Total"}
    )

    df_educacao = pd.merge(
        df_ensino_n_superior, df_ensino_superior, on=["Região"], how="left"
    )
    ...
    # Populacao
    df_populacao_residente = porDataProcess.find_df_by_name(
        dfs, "populacao-residente-por-sexo-e-grupo-etario"
    )
    df_populacao_densidade = porDataProcess.find_df_by_name(
        dfs, "densidade-populacional"
    )
    df_populacao_estrangeira = porDataProcess.find_df_by_name(
        dfs, "populacao-estrangeira-com-estatuto-legal-de-residente"
    )
    ...
    df_populacao_densidade = df_populacao_densidade.rename(
        columns={"Valor": "Qtd.Populacao.Densidade.hab.km"}
    )

    df_populacao_densidade = df_populacao_densidade.drop(columns=["Âmbito Geográfico"])

    df_populacao_estrangeira = df_populacao_estrangeira.rename(
        columns={
    ...
    df_populacao_ = pd.merge(
        df_populacao_residente, df_populacao_densidade, on=["Região"], how="left"
    )
    df_populacao = pd.merge(
        df_populacao_, df_populacao_estrangeira, on=["Região"], how="left"
    )
    ...
    df_seg_crimes_tipo = porDataProcess.find_df_by_name(
        dfs, "crimes-registados-pelas-policias-por-tipo-de-crime"
    )
    df_seg_crimes_taxa = porDataProcess.find_df_by_name(dfs, "taxa_crimes")

    df_seg_crimes_cat = df_seg_crimes_cat.rename(
        columns={
    ...
    df_seg_crimes_tipo = df_seg_crimes_tipo.rename(
        columns={
    ...
    df_seg_crimes_taxa = df_seg_crimes_taxa.rename(
        columns={
            "Regiao": "Região",
    ...
    df_seguranca_ = pd.merge(
        df_seg_crimes_cat, df_seg_crimes_tipo, on=["Região"], how="left"
    )
    df_seguranca = pd.merge(
        df_seguranca_, df_seg_crimes_taxa, on=["Região"], how="left"
    )
    df_seguranca.drop(columns=["Ano_y"], inplace=True)
    df_seguranca = df_seguranca.rename(columns={"Ano_x": "Ano"})

    # Ecomonia
    df_econ_depositos = porDataProcess.find_df_by_name(
        dfs, "depositos-de-clientes-nos-bancos-caixas-economicas-e"
    )
    df_econ_bancos = porDataProcess.find_df_by_name(
        dfs, "estabelecimentos-de-bancos-caixas-economicas-e-caixas"
    )

    df_econ_bancos = df_econ_bancos.rename(
        columns={"Valor": "Qtd.Estabelecimentos.Bancarios"}
    )
    ...
    # Emprego
    df_emp_ramo = porDataProcess.find_df_by_name(
        dfs, "Pessoal-ao-servico-nas-empresas-por-ramo-de-atividade"
    )
    df_emp_desemprego = porDataProcess.find_df_by_name(
        dfs, "desemprego-registado-nos-centros-de-emprego-por-grup"
    )
    df_emp_emprego = porDataProcess.find_df_by_name(
        dfs, "populacao-empregada-por-conta-de-outrem-por-nivel-de"
    )

    df_emp_emprego = df_emp_emprego.rename(
        columns={
    ...
    df_emp_desemprego = df_emp_desemprego.rename(
        columns={
    ...
    df_emp_ramo.columns = df_emp_ramo.columns.str.strip().str.replace(
        "\xa0", " ", regex=False
    )
    df_emp_ramo.columns = [
        re.sub(r"\s+", " ", col.strip()) for col in df_emp_ramo.columns
    ]

    df_emp_ramo = df_emp_ramo.rename(
        columns={
    ...
    # Renda
    df_renda_mensal_escolaridade = porDataProcess.find_df_by_name(
        dfs, "ganho-medio-mensal-por-nivel-de-escolaridade"
    )
    ...
    df_renda_mensal_escolaridade = df_renda_mensal_escolaridade.rename(
        columns={
    ...
    df_renda_poder_compra.rename(
        columns={"Valor": "Qtd.Renda.Poder_de_Compra"}, inplace=True
    )

    df_renda_ = pd.merge(
        df_renda_mensal, df_renda_mensal_escolaridade, on=["Região"], how="left"
    )
    ...
    # Moradia
    df_moradia_arrendamento = porDataProcess.find_df_by_name(
        dfs, "Valor Medio Arrendamento"
    )
    df_moradia_construcoes = porDataProcess.find_df_by_name(
        dfs, "fogos-concluidos-em-construcoes-novas-para-habitacao"
    )
    df_moradia_contrucoes_tipo = porDataProcess.find_df_by_name(
        dfs, "edificios-concluidos-por-tipo-de-obra-e-fim-que-se"
    )

    df_moradia_construcoes = df_moradia_construcoes.rename(
        columns={
    ...
    df_moradia_contrucoes_tipo = df_moradia_contrucoes_tipo.rename(
        columns={
    ...
    df_moradia_arrendamento = df_moradia_arrendamento.rename(
        columns={
            "Regiao": "Região",
            "Codigo": "Código",
    ...
    df_moradia_ = pd.merge(
        df_moradia_construcoes, df_moradia_contrucoes_tipo, on=["Região"], how="left"
    )
    df_moradia = pd.merge(
        df_moradia_, df_moradia_arrendamento, on=["Região"], how="left"
    )
    ...
    df_populacao_emprego_renda = pd.merge(
        df_populacao_emprego, df_renda, on=["Região"], how="left"
    )

    df_moradia = df_moradia.drop(columns=["Ano"])
    df_populacao_emprego_renda_moradia = pd.merge(
        df_populacao_emprego_renda, df_moradia, on=["Região"], how="left"
    )

    df_saude = df_saude.drop(columns=["Ano"])
    df_populacao_emprego_renda_moradia_saude = pd.merge(
        df_populacao_emprego_renda_moradia, df_saude, on=["Região"], how="left"
    )

    df_educacao = df_educacao.drop(columns=["Ano"])
    df_populacao_emprego_renda_moradia_saude_educacao = pd.merge(
        df_populacao_emprego_renda_moradia_saude, df_educacao, on=["Região"], how="left"
    )

    df_economia = df_economia.drop(columns=["Ano"])
    df_populacao_emprego_renda_moradia_saude_educacao_seguranca_economia = pd.merge(
        df_populacao_emprego_renda_moradia_saude_educacao,
        df_economia,
        on=["Região"],
        how="left",
    )
    ...
    df_populacao_emprego_renda_moradia_saude_educacao_seguranca_economia_cultura = (
        pd.merge(
            df_populacao_emprego_renda_moradia_saude_educacao_seguranca_economia,
            df_cultura,
            on=["Região"],
            how="left",
        )
    ...
    df_populacao_emprego_renda_moradia_saude_educacao_seguranca_economia_cultura = df_populacao_emprego_renda_moradia_saude_educacao_seguranca_economia_cultura.loc[
        :,
        ~df_populacao_emprego_renda_moradia_saude_educacao_seguranca_economia_cultura.columns.duplicated(),
    ]
    ...
    colunas_ordenadas = [
        "Ano",
        "Região",
        "Qtd.Populacao.Homens",
        "Qtd.Populacao.Mulheres",
        "Qtd.Populacao.0_4_anos",
        "Qtd.Populacao.5_9_anos",
        "Qtd.Populacao.10_14_anos",
        "Qtd.Populacao.15_19_anos",
        "Qtd.Populacao.20_24_anos",
        "Qtd.Populacao.25_29_anos",
        "Qtd.Populacao.30_34_anos",
        "Qtd.Populacao.35_39_anos",
        "Qtd.Populacao.40_44_anos",
        "Qtd.Populacao.45_49_anos",
        "Qtd.Populacao.50_54_anos",
        "Qtd.Populacao.55_59_anos",
        "Qtd.Populacao.60_64_anos",
        "Qtd.Populacao.65_69_anos",
        "Qtd.Populacao.70_74_anos",
        "Qtd.Populacao.75_79_anos",
        "Qtd.Populacao.80_84_anos",
        "Qtd.Populacao.85_ou_mais_anos",
        "Qtd.Populacao.Total",
        "Qtd.Populacao.Total.Criancas",
        "Qtd.Populacao.Total.Adolescentes",
        "Qtd.Populacao.Total.Jovens_Adultos",
        "Qtd.Populacao.Total.Adultos",
        "Qtd.Populacao.Total.Idosos",
        "Qtd.Populacao.Proporcao.Criancas",
        "Qtd.Populacao.Proporcao.Adolescentes",
        "Qtd.Populacao.Proporcao.Jovens_Adultos",
        "Qtd.Populacao.Proporcao.Adultos",
        "Qtd.Populacao.Proporcao.Idosos",
        "Qtd.Populacao.Densidade.hab.km",
        "Qtd.Populacao.Estrangeira.Africa",
        "Qtd.Populacao.Estrangeira.Angola",
        "Qtd.Populacao.Estrangeira.Cabo_Verde",
        "Qtd.Populacao.Estrangeira.Guine_Bissau",
        "Qtd.Populacao.Estrangeira.Mocambique",
        "Qtd.Populacao.Estrangeira.Sao_Tome_e_Principe",
        "Qtd.Populacao.Estrangeira.America",
        "Qtd.Populacao.Estrangeira.Brasil",
        "Qtd.Populacao.Estrangeira.Asia",
        "Qtd.Populacao.Estrangeira.China",
        "Qtd.Populacao.Estrangeira.India",
        "Qtd.Populacao.Estrangeira.Nepal",
        "Qtd.Populacao.Estrangeira.Europa",
        "Qtd.Populacao.Estrangeira.Espanha",
        "Qtd.Populacao.Estrangeira.Franca",
        "Qtd.Populacao.Estrangeira.Italia",
        "Qtd.Populacao.Estrangeira.Republica_da_Moldova",
        "Qtd.Populacao.Estrangeira.Reino_Unido",
        "Qtd.Populacao.Estrangeira.Romenia",
        "Qtd.Populacao.Estrangeira.Ucrania",
        "Qtd.Populacao.Estrangeira.Total",
        "Qtd.Empregados.Inferior.1.Ciclo",
        "Qtd.Empregados.Basico.1.Ciclo",
        "Qtd.Empregados.Basico.2.Ciclo",
        "Qtd.Empregados.Basico.3.Ciclo",
        "Qtd.Empregados.Secundario",
        "Qtd.Empregados.TecnicoSuperiorProfissional",
        "Qtd.Empregados.Bacharelado",
        "Qtd.Empregados.Licenciatura",
        "Qtd.Empregados.Mestrado",
        "Qtd.Empregados.Doutorado",
        "Qtd.Empregados.Total",
        "Qtd.Desempregados.Faixa.<25",
        "Qtd.Desempregados.Faixa.25-34",
        "Qtd.Desempregados.Faixa.35-44",
        "Qtd.Desempregados.Faixa.45-54",
        "Qtd.Desempregados.Faixa.>55",
        "Qtd.Desempregados.Faixa.Total",
        ...
        "Qtd.Empregos.Ramo.Pesca_Aquicultura",
        "Qtd.Empregos.Ramo.Industrias_Extrativas",
        "Qtd.Empregos.Ramo.Industrias_Transformadoras",
        "Qtd.Empregos.Ramo.Energia_Eletricidade_Gas",
        "Qtd.Empregos.Ramo.Saneamento_Agua_Residuos",
        "Qtd.Empregos.Ramo.Construcao",
        "Qtd.Empregos.Ramo.Promocao_Imobiliaria_Construcao",
        "Qtd.Empregos.Ramo.Engenharia_Civil",
        "Qtd.Empregos.Ramo.Construcao_Especializada",
        "Qtd.Empregos.Ramo.Comercio_Manutencao_Veiculos",
        "Qtd.Empregos.Ramo.Comercio_Reparacao_Veiculos",
        "Qtd.Empregos.Ramo.Comercio_Por_Grosso",
        "Qtd.Empregos.Ramo.Comercio_A_Retalho",
        "Qtd.Empregos.Ramo.Transportes_Armazenagem",
        "Qtd.Empregos.Ramo.Alojamento_Restauracao",
        "Qtd.Empregos.Ramo.Alojamento",
        "Qtd.Empregos.Ramo.Restauracao",
        "Qtd.Empregos.Ramo.Informacao_Comunicacao",
        "Qtd.Empregos.Ramo.Atividades_Imobiliarias",
        "Qtd.Empregos.Ramo.Consultoria_Cientifica_Tecnica",
        "Qtd.Empregos.Ramo.Juridico_Contabilidade",
        "Qtd.Empregos.Ramo.Consultoria_Gestao",
        "Qtd.Empregos.Ramo.Engenharia_Arquitetura_Analises",
        "Qtd.Empregos.Ramo.Servicos_Administrativos_Apoio",
        "Qtd.Empregos.Ramo.Educacao",
        "Qtd.Empregos.Ramo.Saude_Apoio_Social",
        "Qtd.Empregos.Ramo.Artes_Cultura",
        "Qtd.Empregos.Ramo.Outros_Servicos",
        "Qtd.Empregos.Ramo.Total",
        "Qtd.Renda.Media.Mensal",
        "Qtd.Renda.Escolaridade.Inferior.1.Ciclo",
        "Qtd.Renda.Escolaridade.Basico.1.Ciclo",
        "Qtd.Renda.Escolaridade.Basico.2.Ciclo",
        "Qtd.Renda.Escolaridade.Basico.3.Ciclo",
        "Qtd.Renda.Escolaridade.Secundario",
        "Qtd.Renda.Escolaridade.TecnicoSuperiorProfissional",
        "Qtd.Renda.Escolaridade.Bacharelado",
        "Qtd.Renda.Escolaridade.Licenciatura",
        "Qtd.Renda.Escolaridade.Mestrado",
        "Qtd.Renda.Escolaridade.Doutorado",
        "Qtd.Renda.Escolaridade.Total",
        "Qtd.Renda.Poder_de_Compra",
        "Qtd.Habitacao.Quartos.T0_T1",
        "Qtd.Habitacao.Quartos.T2",
        "Qtd.Habitacao.Quartos.T3",
        "Qtd.Habitacao.Quartos.T4_ou_mais",
        "Qtd.Habitacao.Quartos.Total",
        "Qtd.Habitacao.Familiar",
        "Qtd.Habitacao.Outros",
        "Qtd.Habitacao.Ampliacoes_Alteracoes_Reconstrucoes",
        "Qtd.Habitacao.Novas",
        "Qtd.Habitacao.Total",
        "Valor.Habitacao.Arendamento.m2",
        "Valor.Habitacao.Arendamento.100m2",
        "Valor.Habitacao.Arendamento.80m2",
        "Qtd.Hospitais.Publico.Privado",
        "Qtd.Hospitais.Privado",
        "Qtd.Hospitais.Publico",
        "Qtd.Hospitais.Total",
        "Qtd.Farmacias",
        "Qtd.Ensino.NSuperior.Privado",
        "Qtd.Ensino.NSuperior.Publico",
        "Qtd.Ensino.NSuperior.artistica",
        "Qtd.Ensino.NSuperior.basica",
        "Qtd.Ensino.NSuperior.basica.secundaria",
        "Qtd.Ensino.NSuperior.profissional",
        "Qtd.Ensino.NSuperior.secundaria",
        "Qtd.Ensino.NSuperior.Jardim_de_infância",
        "Qtd.Ensino.NSuperior.Total",
        "Qtd.Ensino.Superior.Privado",
        "Qtd.Ensino.Superior.Publico",
        "Qtd.Ensino.Superior.Total",
        "Qtd.Estabelecimentos.Bancarios",
        "Qtd.Depositos.De.Imigrantes",
        "Qtd.Depositos.De.Outros.Clientes",
        "Qtd.Depositos.Total",
        "Qtd.Cinemas",
        "Qtd.Sessões",
    ]
